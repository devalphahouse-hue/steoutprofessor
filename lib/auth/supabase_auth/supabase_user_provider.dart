...
class StepoutProfessorSupabaseUser extends BaseAuthUser {
  StepoutProfessorSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> stepoutProfessorSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = StepoutProfessorSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
